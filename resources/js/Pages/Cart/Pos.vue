...
import {getCurrency, numberFormat, showToast, truncateString, formatQuantity} from "@/Utils/Helper.js";
...
watch(props, (newProps) => {
    if (form.custom_discount.discount_type === "fixed") {
        form.total = numberFormat(newProps.total - form.custom_discount.discount);
        form.paid = numberFormat(newProps.total - form.custom_discount.discount);
    } else {
...
watch(() => form.custom_discount, async (newForm, oldForm) => {
    if (form.custom_discount.discount_type === "fixed") {
        form.total = numberFormat(props.total - form.custom_discount.discount);
        form.paid = numberFormat(props.total - form.custom_discount.discount);
    } else {
...
const insertCartQuantity = (cart, quantity) => {
    const intQuantity = formatQuantity(quantity);
    
    if (cart.quantity === intQuantity || intQuantity < 0) {
        return;
    }
...
const createOrder = () => {
    if (!props.carts.total || props.carts.total === 0) {
        showToast('error', 'Cart is empty! Please add products to cart.');
        return;
    }

    // Reserve a print tab in direct user interaction to avoid popup blockers.
    const printTab = window.open('', '_blank');
    if (printTab) {
        printTab.document.write('<!doctype html><title>Preparing invoice...</title><p style="font-family:Arial,sans-serif;padding:24px;">Preparing invoice...</p>');
    }
    ...
    form.post(route('orders.store'), {
        ...
        onSuccess: (page) => {
            ...
            if (orderId) {
                const printUrl = route('orders.print', orderId);
                                    ...
                                    <div class="flex pb-3 px-3 text-sm">
                                        <p class="flex-grow truncate mr-1">
                                            <span v-if="product.quantity > 0" class="text-xs font-semibold inline-block py-1 px-2 rounded text-emerald-600 bg-emerald-200">
                                                {{ formatQuantity(product.quantity) }}{{ product.unit_type?.symbol }}
                                            </span>
                        ...
                        <div class="lg:w-2/5">
                            ...
                            <div class="flex flex-row items-center justify-between px-5 mt-5">
                                ...
                                <div class="font-semibold">
                                    ...
                                    >Clear({{ formatQuantity(carts.total) }})</span>
                                </div>
                            </div>
                            <!-- end header -->
                            <!-- order list -->
                                ...
                                >
                                    <div class="flex flex-row items-center w-2/5" :title="cart.product.name">
                                        ...
                                        <span class="ml-1 font-semibold text-sm">
                                            ...
                                            <br>
                                            Q: {{ formatQuantity(cart.product.quantity) }} {{ cart.product.unit_type?.symbol }}
                                        </span>
                                    </div>
                                    <div class="flex justify-between">
                                        ...
                                        <input
                                            ...
                                            :value="formatQuantity(cart.quantity)"
                                            step="1"
                                            min="0"
                                        >
                                        <span
                            ...
                            <div class="px-5 mt-3 mb-4">
                                <SubmitButton
                                    ...
                                    :class="!carts.total ? 'cursor-not-allowed opacity-50' : 'hover:bg-emerald-600'"
